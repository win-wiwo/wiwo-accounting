...
export const PrimaryButton = React.forwardRef<HTMLButtonElement, PrimaryButtonProps>(
  ({ className, type = 'button', ...props }, ref) => (
    <button
      ...
      className={cn(
        'premium-primary-btn',
        'inline-flex items-center justify-center gap-2 h-8 rounded-[10px] px-3.5 text-xs font-semibold text-white cursor-pointer',
...
export const GhostButton = React.forwardRef<HTMLButtonElement, GhostButtonProps>(
  ({ className, type = 'button', ...props }, ref) => (
    <button
      ...
      {...props}
      className={cn(
        'inline-flex items-center justify-center gap-2 h-8 rounded-[10px] px-3.5 text-xs font-medium text-zinc-700 cursor-pointer',
